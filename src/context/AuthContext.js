...
export const AuthProvider = ({children}) => {
  ...
  var [lastAnomalyTime, setLastAnomalyTime] = useState([null, null, null]);
      ...
      .then(response => {
        ...
        if (data.length > 0) {
          for (let x = 0; x < data.length; x++) {
            var time = data[x][0].slice(0, 19);
            var power = data[x][1];
            var location = data[x][3];
            if (data[x][2] == 'Anomaly') {
              if (lastAnomalyTime[x] != time) {
                anomalyNotification(power, time, location);
                lastAnomalyTime[x] = time;
  ...
  const anomalyNotification = (pc, time, location) => {
    PushNotification.localNotification({
      ...
      message: `Anomaly Detected in ${location} with the power comsumption of ${pc} at ${time}`, // (required)
    });
  };
